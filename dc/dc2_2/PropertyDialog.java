...
public class PropertyDialog extends JFrame implements ActionListener{
	...
	public PropertyDialog(Property prop) {
		...
		fontComboBox = createJComboBox(1, 0, panel, gbl, gbc, FONTS);
		// Font Size
		createJLabel("Font Size", 0, 1, panel, gbl, gbc);
		fontSizeComboBox = createJComboBox(1, 1, panel, gbl, gbc, FONT_SIZE);
		// Clock Color
		createJLabel("Clock Color", 0, 2, panel, gbl, gbc);
		clockColorComboBox = createJComboBox(1, 2, panel, gbl, gbc, STRING_COLOR);
		clockColorComboBox.addActionListener(this);
		clockColorLabel = createJLabel("■", 2, 2, panel, gbl, gbc);
		// Back Color
		createJLabel("Back Color", 0, 3, panel, gbl, gbc);
		backColorComboBox = createJComboBox(1, 3, panel, gbl, gbc, STRING_COLOR);
		backColorComboBox.addActionListener(this);
		backColorLabel = createJLabel("■", 2, 3, panel, gbl, gbc);
		// cancel button
		createJButton(CANCEL, 0, 4, panel, gbl, gbc).addActionListener(this);
		// ok button
		createJButton(OK, 1, 4, panel, gbl, gbc).addActionListener(this);
